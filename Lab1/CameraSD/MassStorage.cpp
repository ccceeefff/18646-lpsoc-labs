...
void MassStorage::Init()
{
  ...
  unsigned int now = millis();
	do {
		...
		if (abort)
		{
			SerialUSB.print("Cannot initialize the SD card: ");
			switch (err)
			{
				case SD_MMC_ERR_NO_CARD:
					SerialUSB.print( "Card not found\n");
					break;
				case SD_MMC_ERR_UNUSABLE:
					SerialUSB.print( "Card is unusable, try another one\n");
					break;
				case SD_MMC_ERR_SLOT:
					SerialUSB.print( "Slot unknown\n");
					break;
				case SD_MMC_ERR_COMM:
					SerialUSB.print("General communication error\n");
					break;
				case SD_MMC_ERR_PARAM:
					SerialUSB.print("Illegal input parameter\n");
					break;
				case SD_MMC_ERR_WP:
					SerialUSB.print("Card write protected\n");
					break;
				default:
					SerialUSB.print( "Unknown (code ");
					SerialUSB.print(err);
					SerialUSB.println(")");
					break;
			}
	...
	if (mounted != FR_OK)
	{
		SerialUSB.print("Can't mount filesystem 0: code ");
...
const char* MassStorage::CombineName(const char* directory, const char* fileName)
{
	...
	if (directory != NULL)
	{
		while (directory[in] != 0 && directory[in] != '\n')
		{
			...
			{
				SerialUSB.print("CombineName() buffer overflow.");
				out = 0;
			}
		}
	}
...
bool MassStorage::Delete(const char* directory, const char* fileName)
{
	...
	{
		SerialUSB.print("Can't delete file ");
...
bool MassStorage::MakeDirectory(const char *parentDir, const char *dirName)
{
	...
	{
		SerialUSB.print("Can't create directory ");
		SerialUSB.println(location);
		return false;
	}
	return true;
...
bool MassStorage::Rename(const char *oldFilename, const char *newFilename)
{
	...
	{
		SerialUSB.print("Can't rename file or directory ");
		SerialUSB.print(oldFilename);
		SerialUSB.print(" to ");
		SerialUSB.println(newFilename);
		return false;
	}
	return true;
}
...
bool FileStore::Open(const char* directory, const char* fileName, bool write)
{
	...
	if (openReturn != FR_OK)
	{
		SerialUSB.print("Can't open ");
		SerialUSB.print(location);
		SerialUSB.print(" to ");
		SerialUSB.print((writing) ? "write" : "read");
		SerialUSB.print(", error code ");
		SerialUSB.println(openReturn);
		return false;
	}
...
void FileStore::Duplicate()
{
	if (!inUse)
	{
		SerialUSB.print("Attempt to dup a non-open file.\n");
		return;
	}
	++openCount;
}
...
bool FileStore::Close()
{
	if (!inUse)
	{
		SerialUSB.print("Attempt to close a non-open file.\n");
		return false;
	}
	--openCount;
	if (openCount != 0)
...
bool FileStore::Seek(unsigned long pos)
{
	if (!inUse)
	{
		SerialUSB.print("Attempt to seek on a non-open file.\n");
		return false;
	}
	if (writing)
	{
...
unsigned long FileStore::Length() const
{
	if (!inUse)
	{
		SerialUSB.print("Attempt to size non-open file.\n");
		return 0;
	}
	return file.fsize;
}
...
bool FileStore::ReadBuffer()
{
	...
	if (readStatus)
	{
		SerialUSB.print("Error reading file.\n");
		return false;
	}
	bufferPointer = 0;
	return true;
...
bool FileStore::Read(char& b)
{
	if (!inUse)
	{
		SerialUSB.print("Attempt to read from a non-open file.\n");
		return false;
	}
...
bool FileStore::WriteBuffer()
{
	if (bufferPointer != 0)
	{
		...
		if (!ok)
		{
			SerialUSB.print("Cannot write to file. Disc may be full.\n");
			return false;
		}
		bufferPointer = 0;
	}
...
bool FileStore::Write(char b)
{
	if (!inUse)
	{
		SerialUSB.print("Attempt to write byte to a non-open file.\n");
		return false;
	}
	buf[bufferPointer] = b;
	bufferPointer++;
...
bool FileStore::Write(const char* b)
{
	if (!inUse)
	{
		SerialUSB.print("Attempt to write string to a non-open file.\n");
		return false;
	}
	int i = 0;
	while (b[i])
...
bool FileStore::Write(const char *s, unsigned int len)
{
	if (!inUse)
	{
		SerialUSB.print("Attempt to write block to a non-open file.\n");
		return false;
	}
	if (!WriteBuffer())
	{
...
bool FileStore::Flush()
{
	if (!inUse)
	{
		SerialUSB.print("Attempt to flush a non-open file.\n");
		return false;
	}
	if (!WriteBuffer())
	{
